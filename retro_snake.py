...
class SnakeGame:
    def __init__(self):
        # Initialize display with Pi-friendly settings
        try:
            # Try fullscreen first for Pi
            self.screen = pygame.display.set_mode((WINDOW_WIDTH, WINDOW_HEIGHT))
            pygame.display.set_caption("Retro Snake - Raspberry Pi Edition")
            print(f"Display initialized: {WINDOW_WIDTH}x{WINDOW_HEIGHT}")
            ...
            GRID_HEIGHT = WINDOW_HEIGHT // GRID_SIZE
            self.screen = pygame.display.set_mode((WINDOW_WIDTH, WINDOW_HEIGHT))
            print(f"Fallback display: {WINDOW_WIDTH}x{WINDOW_HEIGHT}")
...
if __name__ == "__main__":
    try:
        # Initialize game with debug info
        print("=" * 50)
        ...
        # Check pygame and display info
        print(f"Pygame version: {pygame.version.ver}")
        print(f"SDL version: {pygame.version.SDL}")
        
        # Initialize pygame with specific drivers for Pi
        pygame.display.init()
        pygame.mixer.quit()  # Disable audio to save resources
        
        # Get available display modes
        modes = pygame.display.list_modes()
        print(f"Available display modes: {modes[:3]}...")  # Show first 3
        
        game = SnakeGame()
        print("Game initialized successfully!")
        print("You should see the game window now...")
